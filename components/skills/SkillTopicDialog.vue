...
export interface SkillTopicDialogProps {
  title: string
}

defineProps<SkillTopicDialogProps>()

export interface SkillTopicDialogEmits {
  close: []
}

const emit = defineEmits<SkillTopicDialogEmits>()
...
const onPopState = (event: PopStateEvent): void => {
  if (event.state === null || !event.state.skillTopic) {
    close()
  }
}
...
onMounted(() => {
  ...
  locked.value = true
  window.history.pushState({ skillTopic: true }, '')
